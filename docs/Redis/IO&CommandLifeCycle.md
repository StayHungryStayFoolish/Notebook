...
-   **基本概念**
    ...
    -   **CPU 与寻址**
        ...
        ![CPU 寻址](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/CPU&寻址.svg?raw=true)
...
-   `kernel` 数据准备完成，由 `kernel` 向`用户空间` 拷贝数据，完成后向 `用户进程` 返回 ok。
    ...
    ![Block-IO](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/Blocking-IO-Page.svg?raw=true)
...
- 用户进程再次发起 `recvfrom()` 函数调用，由 `kernel` 向 `用户空间` 拷贝数据，完成后向 `用户进程` 返回 ok。
  ...
  ![NonBlocking-IO](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/NonBlocking-IO-.svg?raw=true)
...
- 用户进程发起 `recvfrom()` 调用，等待由 `kernel` 向 `用户空间` 拷贝数据，完成后向 `用户进程` 返回 ok。
  ...
  ![IO-Multiplexing](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/IO-Multiplexing.svg?raw=true)
...
- ```c
  ...
  ![epoll](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/epoll.svg?raw=true)
...
- `kernel` 准备数据完成后，向 `用户进程` 递交 `SIGIO` 信号，用户进程收到该信号后发起 `recvfrom()` 系统调用，等待 `kernel` 向 `用户空间` 拷贝数据，完成后向 `用户进程` 返回 ok。
  ...
  ![IO-SignalDriven](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/IO-SignalDriven.svg?raw=true)
...
-  `用户进程` 处理 `kernel` 拷贝数据到 `用户空间`。**此阶段 Blocking 状态**
  ...
  ![Asynchronous-IO](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/redis/Asynchronous-IO.svg?raw=true)
