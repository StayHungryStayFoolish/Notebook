...
![Tuning-JVM](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/jvm/jvm-tuning-process.png?raw=true)
...
| **JVM 8 堆外内存设置**                             |                                                    |                                                              |                                                              |
| -XX:MaxDirectMemorySize=1G                         | -XX:MaxDirectMemorySize=1G                         | **默认值 64M**                                               | 当 IO 操作时可能会遇到 `java.lang.OutOfMemoryError: Direct buffer memory` 此时需要调整堆外内存大小以避免类似问题 |
| **设置 `Stack Memory` 空间**                       |                                                    |                                                              |                                                              |
...
| -XX:+DoEscapeAnalysis                              | -XX:+DoEscapeAnalysis                              | **默认开启**                                                 | 开启 `逃逸分析`                                              |
...
| -XX:+EliminateLocks                                | -XX:+EliminateLocks                                | **默认开启，依赖于`-XX:+DoEscapeAnalysis` 选项**             | **开启锁消除**                                               |
| -XX:+EliminateAllocations                          | -XX:+EliminateAllocations                          | **默认开启，依赖于`-XX:+DoEscapeAnalysis` 选项**             | **开启标量替换。对象是聚合量，它又可以被进一步分解成标量，将其成员变量分解为分散的变量，这就叫做标量替换，如果一个对象没有发生逃逸，则不用在 Heap 中创建，只会在 Stack 或者 PC Register 创建它用到的成员标量，节省内存空间，提升性能。上述分配过程属于 Stack Allocation(栈上分配)，可以参考 [JVM 内存模型内存分配策略](http://notebook.bonismo.ink/#/Java/JVM/JVM)** |
|                                                    |                                                    |                                                              |                                                              |
...
![Tuning-size](https://raw.githubusercontent.com/StayHungryStayFoolish/notebook-img/master/img/jvm/Tuning-size.png?raw=true)
