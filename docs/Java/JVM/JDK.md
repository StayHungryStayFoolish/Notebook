...
-   该区域的内存不必是连续的，且可以指定方法区域的大小。
-   所有`类级别的数据`、`运行时常量池(static)`都将存储在此区域。
    -   方法区存储类结构，如`运行时常量池`；字段和方法数据；方法和构造函数的代码，包括在类、实例和接口初始化中使用的特殊方法。
-   **异常情况**
...
#### 2.5 Native Method Stack(本机方法栈)
-   本机方法栈也称为 `C栈`，保存本机方法信息。支持本机方法（方法用 Java 编程语言以外的其他语言编写），通常在`创建每个线程时为每个线程分配`。
-   本机方法堆栈的大小可以是固定的，也可以是动态的。
-   **异常情况**
    -   如果线程中的计算所需的本机方法堆栈超出允许的范围，JVM 将抛出`StackOverflowError`。
    -   如果本机方法堆栈可以动态扩展，并且尝试扩展本机方法堆栈，但可用内存不足。或者可用内存不足，无法为新线程创建初始本机方法堆栈，JVM 将抛出`OutOfMemoryError`。
