...
1. **静态存储分配**
   - 在编译期就能确定每个数据目标在运行时的内存空间需求，因此在编译期间分配固定的内存空间。该分配策略要求代码中部允许有可变数据结构，也不允许嵌套或递归，因为如果出现将导致程序无法计算准确的内存空间需求。
2. **栈式存储分配**
   - 也称为动态存储分配。通过栈结构来实现。在该方案中，程序对内存的需求完全是未知的，只有在运行时才知道。同时规定在进入 `当前栈` 时必须知道  `当前方法` 所需内存空间大小才能够分配其内存。该分配策略按照 `LIFO` 原则进行分配。
3. **堆式存储分配**
   - 同前两者不同，堆式分配专业负责在编译时或 `当前栈` 进入 `当前方法` 无法确定内存需求空间的情况下，分配内存。如分配可变字符串、可变数组、对象实例等。
